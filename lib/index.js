...
class PDFExporter extends EventEmitter {
   ...
   * @param {object} [opts.loggers] Allows client to use its own logging
   *   implementation
   */
  constructor (opts) {
    super()
  ...
  start (options) {
    ...
    electronApp.on('certificate-error',
      (event, webContents, url, error, certificate, callback) => {
        this.emit('certificate-error', { url: url, error: error, certificate: certificate })

        if (options != null && options.insecure === true) {
          this.emit('certificate-error-override')
          console.warn('Certificate-error: ' + error + '. Insecure flag set so will ignore error')

          // Allows errors to continue
          event.preventDefault()
          callback(true)

        } else {
          console.error('Failed to process PDF for url: ' + url + '\nElectron has certificate error: ' + error)
          console.error('Run electron-pdf with --insecure option to ignore *ALL* certificate errors')
          callback(false)
        }
      })
  ...
  createJob (input, output, args, options) {
    ...
    // charge.js interprets the args, but this method should also support raw
    // args
